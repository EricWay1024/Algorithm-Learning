...
int get_distinct(vector<ll> &r) {
    sort(r.begin(), r.end());
    return distance(r.begin(), unique(r.begin(), r.end()));
}

int main() {
#ifdef D
    freopen("POJ-1200.in", "r", stdin);
    clock_t TIMEA = clock();
#endif
    init();
    int n=read(), nc=read();
    ...
    a.init(tmp);
    rep(i, 1, a.l - n + 1) {
        ret1.push_back(a.slice1(i, i + n - 1));
        // ret2.push_back(a.slice2(i, i + n - 1));
    }
    // int ans = max(get_distinct(ret1), get_distinct(ret2));
    int ans = get_distinct(ret1);
    cout << ans << endl;
#ifdef D
    clock_t TIMEB=clock();
