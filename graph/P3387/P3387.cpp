...
int main() {
    ...
    // solve for scc using tarjan
    rep(i,1,n) if (!tarjan.dfn[i]) tarjan.dfs(i);

    // build new graph for scc
    rep(i,1,n) b[belong[i]] += a[i];

    rep(u,1,n) {
        for(auto v: grh[u]) if (belong[u] != belong[v]) {
            ng[belong[u]].insert(belong[v]);
        }
    }

    // topo sort on new graph
    rep(u,1,scctot) {
        for(auto v: ng[u]) {
            indeg[v]++;
        }
    ...
    ll ans = 0;
    rep(u,1,scctot) ans = max(ans, dp[u]);
    cout << ans << endl;
#ifdef D
    clock_t TIMEB=clock();
